...
function getInteresting3Body(minTime=0, initCond=nothing) #in years, defaults to 0
    ...
    while interesting == false
        ...
        plotData, t, m, rad, collisionBool, collisionInds, initV = gen3Body([120, 200], 600000, initCond)
...
function getLims(xNew, yNew, padding, ΔCx, ΔCy, ΔL, ΔR, ΔU, ΔD) #computes limits given padding and offsets
    ...
    dx = maximum(xNew) - minimum(xNew)
    dy = maximum(yNew) - minimum(yNew)
...
function getΔC(target, start, pos, extraDx, extraDy, x, y, padding, tol=0.0001, maxIter=100000) #find center shifts brute force
    ...
    diffxList = [0.0, 0.0, 0.0]
    diffyList = [0.0, 0.0, 0.0]
    xtargList = [0.0, 0.0, 0.0]
    ytargList = [0.0, 0.0, 0.0]
    ΔL, ΔR = extraDx
    ΔU, ΔD = extraDy
    ...
    for i = 1:length(pos)
        ...
        xtargList[i] = rxTarg
        ytargList[i] = ryTarg
        diffxList[i] = diff(rx, rxTarg)
        diffyList[i] = diff(ry, ryTarg)
    end
    diffx, xInd = findmax(diffxList)
    diffy, yInd = findmax(diffyList) #find the one that shifted the most
    Bx = pos[xInd][1]
    By = pos[yInd][2]
    rxTarg = xtargList[xInd]
    ryTarg = ytargList[yInd]
    if diffx < tol && diffy < tol #not that big a difference, just return the initial guess
        return xlims, ylims, ΔCx, ΔCy
    else
        dx = targxlims[2] - targxlims[1]
        dy = targylims[2] - targylims[1]
        function getDir(targxlims, targylims, ΔCx, ΔCy, tol, dx, dy) #figure out which way we need to shift
            acceptXDir = false
            acceptYDir = false
            signdx = diffx < tol ? 0 : -1
            signdy = diffy < tol ? 0 : -1
            counter = 1
            while acceptXDir == false || acceptYDir == false
                ...
                guessX = ΔCx + signdx * dx * tol / 10
                guessY = ΔCy + signdy * dy * tol / 10
                xlims, ylims = getLims(x, y, padding, guessX, guessY, ΔL, ΔR, ΔU, ΔD)
                rx, ry = relative(xlims, ylims, Bx, By)
                newDiffx = diff(rx, rxTarg)
                newDiffy = diff(ry, ryTarg)
                acceptXDir = newDiffx <= diffx
                acceptYDir = newDiffy <= diffy #are we going in the right direction?
                if counter > 2
        ...
        while diffx > tol || diffy > tol #keep guessing in the right direction until we're within the tolerance
            ...
            xlims, ylims = getLims(x, y, padding, guessX, guessY, ΔL, ΔR, ΔU, ΔD)
            rx, ry = relative(xlims, ylims, Bx, By)
            diffx = diff(rx, rxTarg)
            diffy = diff(ry, ryTarg)
            stopX = diffx < tol
            stopY = diffy < tol #compare new guesses and see if we should stop guessing
            counter += 1
            if counter == maxIter
...
function comparePos(orbitOld, orbiting, m, x, y, padding, ΔCx, ΔCy, ΔL, ΔR, ΔU, ΔD) #try to prevent jumps when switching modes
    ...
    inds = [1, 2, 3]
    otherInd = 0 #trying to make generalization to n-bodies easier
    for i = 1:length(inds)
    ...
    if orbiting != 0 #transitioning to orbiting, frame instantaneously wants to shrink
        cx = center(xNew)
        cy = center(yNew)
        ΔL = oldxlims[1] - xlimsOrbit[1]
        ΔR = oldxlims[2] - xlimsOrbit[2]
        ΔU = oldylims[2] - ylimsOrbit[2]
        ΔD = oldylims[1] - ylimsOrbit[1]
        extraDx = [ΔL, ΔR]
        extraDy = [ΔU, ΔD]
        ...
        cx = center(x)
        cy = center(y)
        ΔL = oldxlims[1] - xlimsNorm[1]
        ΔR = oldxlims[2] - xlimsNorm[2]
        ΔU = oldylims[2] - ylimsNorm[2]
        ΔD = oldylims[1] - ylimsNorm[1]
        extraDx = [ΔL, ΔR]
        extraDy = [ΔU, ΔD]
...
function saveFrames(plotData, t, m, rad, collisionBool, collisionInds)
    ...
    if collisionBool == true #this condition makes 2 seconds of slo-mo right before the collision
        ...
        for i = 1:10:600
            ...
            X = [plotData[1][end-(600-i)], plotData[3][end-(600-i)], plotData[5][end-(600-i)]] ./ 1.5e11
            Y = [plotData[2][end-(600-i)], plotData[4][end-(600-i)], plotData[6][end-(600-i)]] ./ 1.5e11
            minX = (min(X[i1], X[i2]) - 1)
            maxX = (max(X[i1], X[i2]))
            minY = (min(Y[i1], Y[i2]) - 1)
            maxY = (max(Y[i1], Y[i2]))
            dx = maxX - minX
            dy = maxY - minY
            ...
            #draw zoom box
            cornersX = [minX, minX + dF + 1]
            cornersY = [minY, minY + dF + 1]
            ...
            s1x, s1y = relative(p[1], 1 / 8 - offset, 7 / 8 - 0.25)
            s2x, s2y = relative(p[1], 1 / 8 + 0.25 - offset, 7 / 8)
            subCornersX = [s1x, s2x]
            subCornersY = [s1y, s2y] #physical coordinates, box in top left
        ...
        for i = 1:15 #make 0.5 s freeze frame ending
            ...
            #collision cam zoom in
            i1, i2 = collisionInds #these are the ones that are colliding, we use them to set the frame limits
            X = [plotData[1][end], plotData[3][end], plotData[5][end]] ./ 1.5e11
            Y = [plotData[2][end], plotData[4][end], plotData[6][end]] ./ 1.5e11
            minX = (min(X[i1], X[i2]) - 1)
            maxX = (max(X[i1], X[i2]))
            minY = (min(Y[i1], Y[i2]) - 1)
            maxY = (max(Y[i1], Y[i2]))
